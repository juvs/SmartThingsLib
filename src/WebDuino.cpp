...
void WebServer::getHeaderCallback(char *value)
{
    memcpy(value, m_subscribeCallback, sizeof(m_subscribeCallback));
}
...
void WebServer::processHeaders()
{
  ...
  while (1)
  {
    ...
    if (expect("CALLBACK:"))
    {
      readHeader(m_subscribeCallback,255);
#if WEBDUINO_SERIAL_DEBUGGING > 1
      Serial.print("\n*** got CALLBACK: of ");
      Serial.print(m_subscribeCallback);
      Serial.print(" ***");
#endif
      continue;
    }
